...
{
	public string similarity, duration;
	public SimilarityResult(string similarity, string duration)
	{
		this.similarity=similarity;
		this.duration=duration;
	}
}
public class AlignmentController : GamePlayController {
	...
	//[SerializeField] Text text_XLeft, text_YLeft, text_XRight, text_YRight, text_Result;
	TCPListener tcp;
	...
	public override void  Update () {
		...
		if (GamePlayController.Instance.IsPlaying()) {
			...
			{
				spawnremainTime += spawnperiod - spawnperiod * Mathf.Clamp01(GamePlayController.GetDifficultyValue(1, 0, 15, 1)) + Random.Range(0.4f, 0.8f);
				SpawnBall();
			}
		}
		if (_finished)
		{
			hat_Controller.gameObject.SetActive(false);
			StopPlay();
			Cursor.visible = true;
			if(_failed)
