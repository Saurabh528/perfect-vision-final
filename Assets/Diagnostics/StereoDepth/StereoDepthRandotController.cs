...
public class StereoDepthRandotController : DiagnosticController
{
    enum DepthMode
    {
        ThreeDepth,
    ...
    [SerializeField]
    GameObject resultPanel, scorePanel, nextRoundButton, seeResultButton;
    [SerializeField]
    Toggle[] togglesDepth, togglesObjCount, togglesRoundCount, togglesDistanceMode;
    [SerializeField]
    private GameObject playPanel, settingPanel;
    [SerializeField] RawImage imageGivenPattern, imagePatternTmpl;
    [SerializeField]
    Texture2D[] patterns;
    [SerializeField]
    Texture2D randotPattern, correctFrame;
    [SerializeField]
    ...
    void StartGamePlay()
    {
        ...
        {
            currentArcsec = PixelToArcSec(Random.Range(1.5f, 2.5f)) + 600;
            arcsecStep = 120;
        }
        else if (roundCountMode == RoundCount.TenRound)
        {
            currentArcsec = PixelToArcSec(Random.Range(1.5f, 2.5f)) + 450;
            arcsecStep = 50;
        }
        else
        {
            currentArcsec = PixelToArcSec(Random.Range(1.5f, 2.5f)) + 380;
            arcsecStep = 20;
        }
        this.ShowNewPattern();
    }
    ...
    void ShowNewPattern()
    {
        ...
        int slotCount = objectCount == ObjectCount.FewObjects ? 9 : 15;
        ...
        Texture2D mergedTexture = DepthMerger.GenerateRandomDotFromShape(randotPattern, slotpatterns[previewIndex], givenPixelOffset);
        ...
        for (int i = 0; i < slotCount; i++)
        {
            ...
            mergedTexture = DepthMerger.GenerateRandomDotFromShape(randotPattern, slotpatterns[i], pixelOffset);
            ...
            if (objectCount == ObjectCount.ManyObjects)
            {
                columnCount = 5;
            }
    ...
    public void OnClickCountButton(GameObject button)
    {
		...
		for (int i = 0; i < slotCount; i++)
		{
			if (correctIndices.Contains(i))
			{
				...
				int col = i % columnCount;
				newObj.GetComponent<RectTransform>().anchoredPosition = tmplPosition + new Vector2(stepX * col, stepY * row);
                RawImage rawImage = newObj.GetComponent<RawImage>();
				rawImage.texture = correctFrame;
        ...
        {
            correct++;
            score += (int)(100f / Mathf.Pow(2f, currentRoundIndex));
            textCorrectStatus.text = correct.ToString();
        }
        else
        {
